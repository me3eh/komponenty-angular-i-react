...
import './App.css';
import {getProducts, addProducts, deleteProductById, editProductById} from "./api";
...
class Products extends React.Component{
    ...
    componentDidMount(){
        getProducts().then(products =>{
            ...
            products.map(product => {
                temparray.push(product.productName);
                ids.push(product.productId);
            });
            this.setState({arrayWithProducts: temparray});
            this.setState({arrayWithIds: ids});
        });
    }

    addProduct(new_item){
        var biggestId = Math.max.apply(null, this.state.arrayWithIds)+1;
        this.setState((prevState) =>({
            arrayWithProducts: [...prevState.arrayWithProducts, new_item],
            arrayWithIds: [...prevState.arrayWithIds, biggestId],
        }));
        let body ={
           productId: biggestId,
           productName: new_item
        }
        addProducts(body);
    }
    ...
    deleteProduct(el){
        ...
        var arrayIds = [...this.state.arrayWithIds];
        var index = array.indexOf(el);
        if(index !== -1){
            array.splice(index, 1);
            this.setState({arrayWithProducts: array});
            deleteProductById(arrayIds[index]);
            arrayIds.splice(index, 1);
            this.setState({arrayWithIds: arrayIds});
        }
    }
    changeNameOfElement(whatIndex, valueToChange){
        ...
        this.setState({arrayWithProducts: array});
        var body = {
            productId: this.state.arrayWithIds[whatIndex],
            productName: valueToChange
        };
        editProductById(body.productId, body);
    }
                    ...
                    <Router>
                        ...
                        <Route
                            ...
                            <ProductDetails 
                                ...
                                indexInDatabase={this.state.arrayWithIds[match.params.id]}
